(function($) {
    ...
    $.jqplot.PieRenderer.prototype.init = function(options, plot) {
        ...
        if (this.highlightColors.length == 0) {
            for (var i = 0; i < this.seriesColors.length; i++) {
                ...
                for (var j = 0; j < 3; j++) {
                    newrgb[j] = sum > 570 ? newrgb[j] * .8 : newrgb[j] + .3 * (255 - newrgb[j]);
    ...
    function calcRPrime(ang1, ang2, sliceMargin, fill, lineWidth) {
        ...
        if (sm > 0 && absang > .01 && absang < 6.282) {
            rprime = parseFloat(sm) / 2 / calcRadiusAdjustment(ang)
        }
        return rprime
    }
    $.jqplot.PieRenderer.prototype.drawSlice = function(ctx, ang1, ang2, color, isShadow) {
        if (this._drawData) {
            ...
            var transx = rprime * Math.cos((ang1 + ang2) / 2);
            var transy = rprime * Math.sin((ang1 + ang2) / 2);
        ...
        for (var i = 0; i < gd.length; i++) {
            ...
            if (this.showSlice[i]) {
                ...
                if (this.showDataLabels && gd[i][2] * 100 >= this.dataLabelThreshold) {
                    var fstr, avgang = (this._sliceAngles[i][0] + this._sliceAngles[i][1]) / 2,
                        label;
